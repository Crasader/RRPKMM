...
BloodData BloodTool::getContentBlood(){
	...
	int powerNum = NTGameSave::sharedGameSave()->getDataById(_GS_HEART);

	int contentTime = getContentSec();

	if(powerNum < 5){
//Jerry--10
        int NextTime = pSava->getIntegerForKey("preTime",0) +60 * 10 - 1;

		if(NextTime - contentTime < 0){

			pSava->setIntegerForKey("preTime",contentTime);
			NTGameSave::sharedGameSave()->updateValue(_GS_HEART,powerNum+1);
		}

		return BloodData(BloodTime(NextTime - contentTime),powerNum);
	}else{

		return BloodData(BloodTime(),powerNum);
	}
} 
